...
function updateChildren(oldChildren, children, patches, patch, index) {
  ...
  outer: while (
    ...
    while (oldStartChild.key === startChild.key) {
      updateNode(oldStartChild, startChild, patches, indexMap[oldStartChild])
      oldStartIndex++
      startIndex++
    ...
    while (oldEndChild.key === endChild.key) {
      updateNode(oldEndChild, endChild, patches, indexMap[oldEndChild])
      oldEndIndex--
      endIndex--
      if (oldStartIndex > oldEndIndex || startIndex > endIndex) {
        break outer
    ...
    while (oldStartChild.key === endChild.key) {
      ...
      updateNode(oldStartChild, endChild, patches, indexMap[oldStartChild])
